...
def plot_concept_top50(
        args, val_loader, model, whitened_layers, path_plots,
        print_other=False, activation_mode='pool_max'):
    '''This function finds the top 50 images that gets the greatest activations
    with respect to the concepts.
        Concept activation values are obtained based on iternorm_rotation
    module outputs.
        Since concept corresponds to channels in the output, we look for the
    top50 images whose kth channel activations are high.'''
    ...
    for layer in layer_list:
        ...
        if layer <= layers[0]:
            ...
            model.layer3[layer - layers[0] - layers[1] - 1].bn1\
                .register_forward_hook(hook)
        elif layer <= layers[0] + layers[1] + layers[2] + layers[3]:
            model.layer4[layer - layers[0] - layers[1] - layers[2] - 1].bn1\
                .register_forward_hook(hook)
...
def plot_correlation(args, val_loader, model, layer, path_plots):
    dst = path_plots + 'correlation_matrix/'
    if not os.path.exists(dst):
        os.mkdir(dst)
